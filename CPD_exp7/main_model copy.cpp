...
unsigned int tStop = 50000;
unsigned int CPD_time = 2000;
float dt =2;
    ...
    for(int t =0;t<int(tStop/dt);t++){
        ...
        if (t == int(CPD_time/dt) ){
            if (CPD_flag==1){
                ...
                for(int i = 0; i <cell_num; i++){
                    if( (cells[i][1]>ydim/2-300*pow(10,-6)) &&(cells[i][1]<ydim/2+300*pow(10,-6))  &&(cells[i][2] < 35*pow(10,-6)) ){
