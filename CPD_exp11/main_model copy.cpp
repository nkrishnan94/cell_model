    ...
    for(int t =0;t<int(tStop/dt);t++){
        ...
        for(int i = 0; i <cell_num; i++){
            if (cells[i][2]>-1){
                ...
                if((cells[i][1] + dt*forces[i][1]/fric + pow(2*Dc*dt,.5)*distribution(generator) < a*R )&&(t>int(CPD_time/dt) )){
                    //forces[i][1] = -forces[i][1];
                    cells[i][1] = a*R*1.2;
                }
                    
                if((cells[i][1] + dt*forces[i][1]/fric + pow(2*Dc*dt,.5)*distribution(generator) - a*R >ydim)&&(t>int(CPD_time/dt) )){
                    //forces[i][1] = -forces[i][1];
                    cells[i][1] = ydim - a*R*1.2;
                    //forces[i][1] =0;
