...
float s_c = .5*pow(10,6);

float s_a = .5*pow(10,6);
...
int main(){
    ...
    cells[1][0]= xdim/2;
    cells[1][1] = a*R + 1.5*a*R;
    cells[1][2]= a*R;
    cells[1][3] = R;

    cells[2][0]= xdim/2;
    cells[2][1] = a*R + 1.5*a*R +1.9*a*R;
    cells[2][2]= zdim/2;
    cells[2][3] = R;*/
        ...
        for(int y =0; y<ycells;y++){
            for(int x =0; x<xcells;x++){
                if(cell_count<cell_num){
                    ...
                    cells[cell_count][1] = double(1.1*a*R+y*y_space);
                    cells[cell_count][2]= double(1.1*a*R+z*z_space);
    ...
    for(int t =0;t<int(tStop/dt);t++){
        ...
        if(t<int(CPD_time/dt)){
            ...
            Dc = 0.01;
            Kc= 6.6*pow(10,-9);//*pow(10,0); N/um
            deld = 1*2*a*R; //um


			Ka= 0*pow(10,-8);//*pow(10,0); N/um	
            ...
            Dc = 0.001;
            deld = 1.4*2*a*R; //um
            Kc= 2.2*pow(10,-9);//*pow(10,0); N/um
            Ka= 2.2*pow(10,-9);//*pow(10,0); N/um	
        ...
        for(int k = 0; k <cell_num; k++){
            ...
            if (cells[i][2]>-1){
                ...
                if( (cells[i][1] + dt*forces[i][1]/fric + pow(2*0*dt,.5)*noise_terms[i][1] < a*R )){
                	//forces[i][1] = -forces[i][1];


                	//cells[i][1] = a*R + abs(dt*forces[i][1]/fric + pow(2*Dc*dt,.5)*noise_terms[i][1]);
                	cells[i][1] = a*R;
                	//cout<<"hi"<<endl;
            	} else if(cells[i][1] + dt*forces[i][1]/fric + pow(2*0*dt,.5)*distribution(generator) >(ydim - a*R) ) {
                    forces[i][1] = -forces[i][1];
                    //forces[i][1] = -forces[i][1];
                    //cells[i][1] = ydim - a*R - abs(dt*forces[i][1]/fric + pow(2*Dc*dt,.5)*noise_terms[i][1]);
                    //forces[i][1] =0;

                    cells[i][1] = ydim - 1.5*a*R;
                ...
                if(((cells[i][2] + dt*forces[i][2]/fric + pow(2*0*dt,.5)*noise_terms[i][2] >= ( ydim - a*R )  ) && (t<int(CPD_time/dt)) )){
                    cells[i][2] = ydim - a*R-abs( dt*forces[i][2]/fric + pow(2*Dc*dt,.5)*noise_terms[i][2] );
                    //cells[i][2] = a*R;
                    //forces[i][2] =-forces[i][2];
                    //cells[i][2] = (ydim-a*R)-abs( dt*forces[i][2]/fric + pow(2*Dc*dt,.5)*noise_terms[i][2] );

                } else if(((cells[i][2] + dt*forces[i][2]/fric + pow(2*0*dt,.5)*noise_terms[i][2] < a*R )  ) ){
        ...
        if ((t % record_time)==0){
            ...
            for(int i = 0; i <cell_num; i++){
                ...
                fproft<< setprecision(32)<< fixed << i << ", " << cells[i][0]*pow(10,6) << ", " << cells[i][1]*pow(10,6) << ", " << cells[i][2]*pow(10,6)<<", " << cells[i][3]*pow(10,6)<<", " << forces[i][0]*pow(10,6)<< ", "<<forces[i][1]*pow(10,6)<< ", "<<forces[i][2]*pow(10,6)<< endl;
        ...
        if (t == int(CPD_time/dt) ){
            if (CPD_flag==1){
                ...
                for(int i = 0; i <cell_num; i++){
                    if( (cells[i][1]>ydim/2-400*pow(10,-6)) &&(cells[i][1]<ydim/2+400*pow(10,-6))  &&(cells[i][2] < 50*pow(10,-6)) ){
